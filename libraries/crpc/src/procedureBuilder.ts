...
import { getParseFn, inferParser, Parser } from "./parser";
...
type inferAsyncIterable<TOutput> = TOutput extends AsyncIterable<
    infer $Yield,
    infer $Return,
    infer $Next
>
    ...
    : never;
type inferSubscriptionOutput<TOutput> = TOutput extends AsyncIterable<any>
    ? AsyncIterable<
          inferAsyncIterable<TOutput>["yield"],
...
> {
    ...
    input<$Parser extends Parser>(
        schema: TInputOut extends UnsetMarker
            ? $Parser
            : inferParser<$Parser>["out"] extends
    ...
    /**
     * Type the input of the procedure without validating it
     */
    $input<TInput>(): ProcedureBuilder<
        TContext,
        TMeta,
        TContextOverrides,
        IntersectIfDefined<TInputIn, TInput>,
        IntersectIfDefined<TInputOut, TInput>,
        TOutputIn,
        TOutputOut,
        TCaller
    >;
    ...
    subscription<$Output extends AsyncIterable<any, void, any>>(
        ...
        : SubscriptionProcedure<{
              ...
              output: inferSubscriptionOutput<
                  DefaultValue<TOutputOut, $Output>
              >;
          }>;
    ...
    const builder: AnyProcedureBuilder = {
        ...
        unstable_concat(builder) {
            return createNewBuilder(
                _def,
                (builder as AnyProcedureBuilder)._def
            );
        },
        query(resolver) {
        ...
        subscription(
            resolver: ProcedureResolver<any, any, any, any, any, any>
        ) {
            return createResolver(
...
async function callRecursive(
    ...
    try {
        ...
        return {
            ok: false,
            error: getCRPCErrorFromUnknown(cause),
            marker: middlewareMarker,
        };
    }
}
